...
if(isset($_POST['action']) && $_POST['action'] == 'listRecords') {
	...
	if(!empty($get_data))
	{
		...
		foreach($get_data as $key => $val){ 
			...
			$rows[] = '<span data-bs-toggle="tooltip" data-id="'.$val['ticket_id'].'" onclick="get_id(this);">ID <b>#</b>'.$val['ticket_id'].'<br>Word Count:&nbsp'.($val['wordcount']).'<br>Date added:&nbsp'.($dateAdded).'<br>Created By:&nbsp'.($val['emp_name']).'<br>Cost:&nbsp'.$ticket_value.' '.$val['currency'].'</span>';
...
if(!empty($_POST['action']) && $_POST['action'] == 'get_account_records') {
	...
	foreach($get_data as $key => $val){ 
		$rows = array();
		$ticket_value = round((($val['wordcount'] * $val['rate'])/ $val['word_limit']),2)+ (int)$val['extra_amount'];
		$rows[] = '<span data-bs-toggle="tooltip" data-id="'.$val['ticket_id'].'" onclick="get_id(this);">ID <b>#</b>'.$val['ticket_id'].'<br>Word Count:&nbsp'.($val['wordcount']).'<br>Cost:&nbsp'.$ticket_value.' '.$val['currency'].'</span>';
		$rows[] = '<span><em class="ni ni-user"></em>&nbsp'.$val['customer_name']."<br><em class='icon ni ni-mobile'></em>&nbsp<a href = 'https://wa.me/".$val['phone_number']."'>".$val['phone_number']."</a><br><em class='icon ni ni-mail-fill'></em>&nbsp<a href = 'mailto:".$val['customer_email']."'>".$val['customer_email'].'</a></span>';
		...
		$btn .= '<a href=html/?action=payment_details&ticketId='.$val['ticket_id'].' class="btn btn-trigger btn-icon dropdown-toggle" target="_blank"><div class="badge badge-circle bg-danger">'.$val['payment_pending'].'</div><em class="icon ni ni-clip"></em></a>';
		}
		else{
			$btn .= '<a href=html/?action=payment_details&ticketId='.$val['ticket_id'].' class="btn btn-trigger btn-icon dropdown-toggle" target="_blank"><em class="icon ni ni-clip"></em></a>';
		}
...
if(!empty($_POST['action']) && $_POST['action'] == 'getAssignDetails') {
	...
	if($_SESSION['emp_role'] == "project_manager")
	{
		...
		echo json_encode(array("response"=>'Fetch Successfully', "status" => 'success',  "data1"=>$get_details));
	}
}
...
// 	$emp_desc = select_query("employees_master",array("emp_name", "emp_phone_no", "emp_role", "manager_id"),array("emp_id" => "$userId"));
// 	echo json_encode(["response"=>'Fetch Successfully', "status" => 'success', 'data1'=>$emp_desc, 'data2'=>$emp_details]);
